...
import { isClientAuthEnabled } from './auth/config';
import { getStoredTenant } from './auth/storage';

const BASE_URL = '/api/controlplane';
const authEnabled = isClientAuthEnabled();
...
function buildHeaders(extra?: Record<string, string>) {
  ...
  if (tenant) {
    headers['x-umbra-tenant-id'] = tenant;
  }
  return headers;
}

function getClient() {
  return createApiClient({ baseUrl: BASE_URL, headers: buildHeaders() });
}
